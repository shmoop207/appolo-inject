...
class Injector {
    ...
    _prepareProperties(definition) {
        ...
        for (let i = 0, length = (definition.inject ? definition.inject.length : 0); i < length; i++) {
            ...
            if (dto.ref) {
                let factoryRef = dto.ref + this.FACTORY_POSTFIX, factoryDef = this.getDefinition(factoryRef), refDef = this.getDefinition(dto.ref), localDef = this._definitions[dto.ref], localFactoryRef = localDef && localDef.injector && localDef.refName ? localDef.refName + this.FACTORY_POSTFIX : factoryRef, localInjectorDef = localDef && localDef.injector && localDef.injector.getDefinition(localFactoryRef), factory;
