...
class Injector {
    ...
    _injectPropertiesAndLookUpMethods(object, objectDefinition, objectId) {
        ...
        for (let i = 0, length = (properties ? properties.length : 0); i < length; i++) {
            ...
            injectObject = null;
            if (prop.array) {
                injectObject = _.map(prop.array, (propObj) => propObj.value || this._getByParamObj(propObj, propObj.ref));
            }
                ...
                injectObject = util_1.Util.createDelegate(this._getByParamObj, this, [prop, prop.factoryMethod]);
            }
            if (injectObject) {
