...
var Injector = Class.define({
    ...
    addDefinition: function (objectId, definition) {
    ...
    _createObjectInstance: function (objectID, objectDefinition, runtimeArgs) {
        ...
        if (!newObjectInstance) {
            ...
            for (i = 0; i < args.length; i++) {
                ...
                argValue = _.has(arg, 'value')
                    ? arg.value
                    : this._createObjectInstance(arg.ref, this._definitions[arg.ref]);
            ...
            try {
                ...
                if (typeof objectDefinition.type === 'function') {
                    ...
                    //newObjectInstance = new (Function.prototype.bind.apply(objectDefinition.type,[null].concat(argumentInstances)));
        ...
        if (!objectDefinition.$propertiesGenerated) {
            ...
            if (objectDefinition.inject) {
                for (i = 0, length = objectDefinition.inject.length; i < length; i++) {
                    var injectable = objectDefinition.inject[i];

                    if (typeof (injectable) == 'object') {
                        properties.push(injectable);
                    }

                    else if (typeof (injectable) == 'string') {
                        properties.push({name: injectable, ref: injectable});
                    }
