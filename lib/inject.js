...
exports.createContainer = exports.Injector = void 0;
...
class Injector {
    ...
    async _loadFactoryInject(def, refs) {
    ...
    _addSingletonAliases(def, instance, checkFactory = true) {
    ...
    _populateAliasFactory(definition, objectId) {
        for (let i = 0, length = definition.aliasFactory ? definition.aliasFactory.length : 0; i < length; i++) {
            ...
            let delegateFn = util_1.Util.createDelegate(((definition.bootstrapMethodAsync || definition.initMethodAsync) ? this._createFactoryMethodAsync : this._createFactoryMethod), this, [objectId, this]);
            delegateFn.type = definition.type;
            util_1.Util.mapPush(this._aliasFactory, aliasName, delegateFn);
        }
    }
    _createFactoryMethod(objectId, injector, runtimeArgs) {
    ...
    async _createFactoryMethodAsync(objectId, injector, runtimeArgs) {
    ...
    _invokeBootStrapMethod(instance, definition) {
        if (instance[IsWiredSymbol]) {
            return;
        }
    ...
    _injectPropertiesAndLookUpMethods(object, objectDefinition, objectId) {
        ...
        for (let i = 0, length = (properties ? properties.length : 0); i < length; i++) {
            ...
            if (prop.array) {
                object[prop.name] = (prop.array || []).map((propObj) => propObj.value || this._getByParamObj(propObj, propObj.ref));
            }
            else if (prop.dictionary) {
                let injectObject = {};
                (prop.dictionary || []).forEach((propObj) => injectObject[propObj.key] = propObj.value || this._getByParamObj(propObj, propObj.ref));
                ...
                object[prop.name] = util_1.Util.createDelegate(this._createFactoryMethod, this, [prop.factoryMethod, prop.injector || this]);
            }
            else if (prop.factoryMethodAsync) {
                object[prop.name] = util_1.Util.createDelegate(this._createFactoryMethodAsync, this, [prop.factoryMethodAsync, prop.injector || this]);
            }
            else if (prop.lazyFn) {
