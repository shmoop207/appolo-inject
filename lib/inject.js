...
class Injector {
    ...
    _prepareProperties(definition) {
        ...
        for (let i = 0, length = (definition.inject ? definition.inject.length : 0); i < length; i++) {
            ...
            if (dto.ref) {
                let refDef = this.getDefinition(dto.ref), localDef = this._definitions[dto.ref], localInjectorDef = localDef && localDef.injector && (localDef.injector.getDefinition(localDef.refName || localDef.id)), factory;
