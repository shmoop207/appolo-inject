...
var Injector = class {
    ...
    initialize(options) {
        ...
        _.forEach(this._options.definitions, (value, key)=>this._definitions.set(kay, value))
    ...
    get(objectID, runtimeArgs, ignoreFactory) {
        ...
        if (this._definitions.has(objectID + FACTORY_POSTFIX) && !ignoreFactory) {
            return this.getObject(objectID + FACTORY_POSTFIX).get();
        }

        var instance = this._instances.get(objectID);
    ...
    removeInstance(objectId) {
        ...
        this._instances.remove(objectId);
    ...
    _populateAliasFactory(definition,objectId){
        _.forEach(definition.aliasFactory, aliasName =>{
            var delegateFn  = this._createDelegate(this.getObject, this, [objectId]);
    ...
    _getProperties(objectDefinition) {
        ...
        if (!objectDefinition.$propertiesGenerated) {
            ...
            _.forEach(objectDefinition.inject, (injectable) => properties.push(_.isString(injectable) ? ({name: injectable, ref: injectable}) : injectable));
    ...
    _injectFactoryObject(object, objectId) {
        ...
        var factoryData = this._factories.get(objectId);
